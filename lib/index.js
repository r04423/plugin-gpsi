...
const DEFAULT_METRICS_PAGESUMMARY = ['score.*', 'loadingExperience.*'];
  ...
  processMessage(message, queue) {
    ...
    switch (message.type) {
      ...
      case 'url': {
        ...
        return analyzer
          .analyzeUrl(url, log, this.options, proxy)
          .then(result => {
            log.info(
              'Got ' + url + ' analysed from Google Page Speed Insights'
            );
            ...
            queue.postMessage(
              ...
              make('gpsi.pageSummary', pageSummary.repackage(result), {
                url,
                group
              })
            );
            ...
            if (
              ...
              this.summaries[group] = summary.repackage(result);
            }
          });
      }
